...
about:
  ...
  links:
    - icon: envelope-fill
      text: Email
      href: "mailto:andrew.proctor@econ.lmu.de"
    - icon: github
      text: Github
...
<h2 style="cursor: pointer;" onclick="toggleInlineDescription()">
  ...
  <span id="inline-description" style="display: none; margin-left: 0.5rem; color: #6c757d; vertical-align: middle;">
    <a class="btn btn-primary" href="/assets/AndrewProctor_CV.pdf" role="button">PDF</a>
  </span>
</h2>
...
document.addEventListener("DOMContentLoaded", () => {
  // Wrap the CV toggle to close details first
  if (typeof toggleInlineDescription === "function") {
    const _origToggle = toggleInlineDescription;
    window.toggleInlineDescription = function() {
      // Close all open <details>
      document.querySelectorAll("details[open]").forEach(d => d.open = false);
      // Hide inline description icon update will be done in listener
      // Call original toggle
      _origToggle();
    };
  }
  // Collapse logic for <details> and hide CV
  document.querySelectorAll("details").forEach(detail => {
    detail.addEventListener("toggle", () => {
      if (!detail.open) return;
      // Close all other <details> not ancestors/descendants
      document.querySelectorAll("details").forEach(other => {
        if (other === detail) return;
        if (other.contains(detail) || detail.contains(other)) return;
        other.open = false;
      });
      // Hide CV inline description
      const desc = document.getElementById("inline-description");
      const icon = document.getElementById("toggle-icon");
